...
function disableOnLoad(disable = true) {
  if (disable) {
    ...
    // remove loading opacity from messages container
...
function makeSeen(status) {
  ...
  // remove unseen counter from contacts list
  $(".messenger-list-item[data-contact=" + getMessengerId() + "]")
    .find("tr>td>b")
    .remove();
...
function deleteConversation(id) {
  $.ajax({
    ...
    success: (data) => {
      ...
      if (!data.deleted)
        return alert(
          "Error occurred, messages can not be deleted!"
        );
...
function deleteMessage(id) {
  $.ajax({
    ...
    success: (data) => {
      ...
      if (!data.deleted) {
        console.error(
          "Error occurred, message can not be deleted!"
        );
      }
...
$(document).ready(function () {
  ...
  $(".messenger-listView-tabs a").on(
    ...
    function () {
      ...
      $(".messenger-listView-tabs a").removeClass(
        "active-tab"
      );
  ...
  $("body").on(
    ...
    ".messenger-list-item",
    function () {
      $(".messenger-list-item").removeClass(
        "m-list-active"
      );
   ...
   *---------------------------------------------------------
   * CORREÇÃO:
   * Abrir usuário encontrado pela busca
   *---------------------------------------------------------
   */
  $("body").on(
    "mousedown",
    ...
    function (e) {
      ...
      if (!dataId) {
        console.error(
          "ID do usuário encontrado na busca não foi localizado."
        );
        return;
      }

      console.log(
        "Abrindo conversa pelo resultado da busca. ID:",
        dataId
      );
  ...
  // On upload attachment input change
  $("body").on(
    "change",
    ".upload-attachment",
    (e) => {
  ...
  $(".messenger-search").on(
    ...
    function () {
      ...
      $('.messenger-tab[data-view="search"]')
        .show();
    }
  );
   ...
   *---------------------------------------------------------
   * CORREÇÃO:
   * Não esconder a busca imediatamente no blur.
   * O mousedown acima abre a conversa antes do blur.
   *---------------------------------------------------------
   */
  $(".messenger-search").on(
    "blur",
...
/**
 *-------------------------------------------------------------
 * Resize messaging area when resize viewport.
 *-------------------------------------------------------------
 */
var resizeTimeout;
